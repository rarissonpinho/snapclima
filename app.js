...
const api_key = "da411e9c0d5201d464b0923f4d40e406"
...
navigator.geolocation.getCurrentPosition(
    (position) => {
        ...
        getCurrentLocationWether(lat, lon)
    },
    (err) => {
        if (err.code === 1) {
...
function getCurrentLocationWether(lat, lon) {
    
    fetch(`https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${lon}&units=metric&lang=pt_br&appid=${api_key}`).then((response) => response.json())
